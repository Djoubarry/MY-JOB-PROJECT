...
exports.getJobs = async (req, res) => {
    ...
    try {
        ...
        if (userId) {
            // Saved Jobs 
            const saveJobs = await SavedJob.find({ jobseeker: userId }).select('job');
            saveJobs = saveJobs.map((s) => String(s.job));
...
exports.getJobsEmployer = async (req, res) => {
      console.log("[1] Fonction appelée"); // S'affiche-t-il ?
    try {
        ...
        const jobs = await Job.find({ company: userId})
            ...
            .lean(); // .lean() makes jobs plainjs Objects so we can add new fiels
...
// @desc Toggle Close Status for a job (Emplyer only)
